...
describe('requestAllBooksEndpoint function', () => {
	beforeEach(() => {
		fetch.resetMocks();
	});
	...
	it('resolves the correct json on successful request', () => {
		...
		return requests.requestAllBooksEndpoint(endpoint2, params).then((data) => {
			...
			mocks.forEach((mock, i) => {
				expect(mock).toHaveBeenCalledWith(`http://wordpress.test/wp-json/wp/v2/${endpoint}/${endpoint2}?context=view&page=${i + 1}&per_page=${per_page}`);
			});
		});
	});
	...
	it('rejects request on error', () => {
		...
		return requests.requestAllBooksEndpoint(endpoint2, params).catch((error) => {
			expect(error).toBe(statusText);
			expect(mock).toHaveBeenCalledWith(`http://wordpress.test/wp-json/wp/v2/${endpoint}/${endpoint2}?context=view&page=1&per_page=100`);
		});
	});
});