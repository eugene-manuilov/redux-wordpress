...
describe('requestAllBooksEndpointById function', () => {
	beforeEach(() => {
		fetch.resetMocks();
	});
	...
	it('resolves the correct json on successful request', () => {
		...
		return requests.requestAllBooksEndpointById(id, endpoint2, params).then((data) => {
	...
	it('rejects request on error', () => {
		...
		return requests.requestAllBooksEndpointById(id, endpoint2, params).catch((error) => {
